...
function isTypeReadonlyArrayOrTuple(checker, type, seenTypes) {
    function checkTypeArguments(arrayType) {
        ...
        if (typeArguments.some(typeArg => isTypeReadonlyRecurser(checker, typeArg, seenTypes) === 2 /* Mutable */)) {
            return 2 /* Mutable */;
        }
        return 3 /* Readonly */;
    }
...
function isTypeReadonlyObject(checker, type, seenTypes) {
    ...
    if (properties.length) {
        ...
        for (const property of properties) {
            const propertyType = _1.nullThrows(
                checker.getTypeOfPropertyOfType(type, property.getName()),
                _1.NullThrowsReasons.MissingToken(`property "${property.name}"`, 'type'),
            );
            ...
            if (isTypeReadonlyRecurser(checker, propertyType, seenTypes) === 2 /* Mutable */) {
                return 2 /* Mutable */;
            }
        }
    }
