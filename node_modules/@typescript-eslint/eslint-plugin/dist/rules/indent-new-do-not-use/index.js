    ...
    create(context, [userIndent, userOptions]) {
        ...
        function addBlocklessNodeIndent(node) {
            if (node.type !== experimental_utils_1.AST_NODE_TYPES.BlockStatement) {
                ...
                while (
                    util_1.isOpeningParenToken(sourceCode.getTokenBefore(firstBodyToken)) &&
        ...
        const baseOffsetListeners = {
            ...
            IfStatement(node) {
                ...
                if (node.alternate && node.alternate.type !== experimental_utils_1.AST_NODE_TYPES.IfStatement) {
            ...
            ImportDeclaration(node) {
                if (
                    node.specifiers.some(
                        specifier => specifier.type === experimental_utils_1.AST_NODE_TYPES.ImportSpecifier,
                    )
                ) {
                ...
                const fromToken = sourceCode.getLastToken(
                    node,
                    token => token.type === experimental_utils_1.AST_TOKEN_TYPES.Identifier && token.value === 'from',
                );
                ...
                if (fromToken) {
                    const end =
                        semiToken && semiToken.range[1] === sourceToken.range[1] ? node.range[1] : sourceToken.range[1];
            ...
            'MemberExpression, JSXMemberExpression, MetaProperty'(node) {
                ...
                const firstPropertyToken = isComputed ? firstNonObjectToken : secondNonObjectToken;
                if (isComputed) {
