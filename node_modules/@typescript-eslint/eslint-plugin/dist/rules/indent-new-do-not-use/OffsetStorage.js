...
class OffsetStorage {
    ...
    setDesiredOffsets(range, fromToken, offset = 0, force = false) {
        ...
        const fromTokenDescriptor = fromTokenIsInRange ? this.getOffsetDescriptor(fromToken) : null;
    ...
    getDesiredIndent(token) {
        if (!this.desiredIndentCache.has(token)) {
            if (this.ignoredTokens.has(token)) {
                ...
                this.desiredIndentCache.set(
                    token,
                    (offsetInfo.from ? this.getDesiredIndent(offsetInfo.from) : '') + this.indentType.repeat(offset),
                );
            }
        }
