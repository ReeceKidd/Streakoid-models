            ...
            ':matches(CallExpression, OptionalCallExpression) > :matches(MemberExpression, OptionalMemberExpression).callee'(
                ...
                if (checker.isArrayType(calleeObjType)) {
                    context.report({
                        ...
                        node: secondArg,
                        fix: fixer => [
                            fixer.removeRange([secondArg.range[0], secondArg.expression.range[0]]),
                            fixer.removeRange([secondArg.expression.range[1], secondArg.range[1]]),
                            fixer.insertTextAfter(
                                callee,
                                `<${context.getSourceCode().getText(secondArg.typeAnnotation)}>`,
                            ),
                        ],
                    });
                    return;
