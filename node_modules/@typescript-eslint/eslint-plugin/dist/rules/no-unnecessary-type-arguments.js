    ...
    create(context) {
        ...
        function checkTSArgsAndParameters(esParameters, typeParameters) {
            ...
            context.report({
                ...
                fix: fixer =>
                    fixer.removeRange(
                        i === 0 ? esParameters.range : [esParameters.params[i - 1].range[1], arg.range[1]],
                    ),
            });
        }
        return {
...
function getTypeParametersFromType(type, checker) {
    ...
    return util_1.findFirstResult(sym.declarations, decl =>
        tsutils.isClassLikeDeclaration(decl) || ts.isTypeAliasDeclaration(decl) || ts.isInterfaceDeclaration(decl)
            ? decl.typeParameters
            : undefined,
    );
}
