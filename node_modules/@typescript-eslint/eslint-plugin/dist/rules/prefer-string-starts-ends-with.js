...
exports.default = util_1.createRule({
    ...
    create(context) {
        ...
        function isEqualityComparison(node) {
            return (
                node.type === experimental_utils_1.AST_NODE_TYPES.BinaryExpression && EQ_OPERATORS.test(node.operator)
            );
        }
        /**
        ...
        function isLengthExpression(node, expectedObjectNode) {
            if (
                ...
                return (
                    util_1.getPropertyName(node, globalScope) === 'length' &&
        ...
        function* fixWithRightOperand(fixer, node, kind, isNegative, isOptional) {
            ...
            const leftNode =
                node.left.type === experimental_utils_1.AST_NODE_TYPES.CallExpression ||
        ...
        return {
            ...
            [[
                ...
                let indexNode = null;
                if (
                    parentNode.type === experimental_utils_1.AST_NODE_TYPES.CallExpression ||
            ...
            [[
                ...
                const isEndsWith =
                    callNode.arguments.length === 1 ||
                    (callNode.arguments.length === 2 && isLengthExpression(callNode.arguments[1], node.object));
                const isStartsWith =
                    !isEndsWith && callNode.arguments.length === 2 && isNumber(callNode.arguments[0], 0);
                if (!isStartsWith && !isEndsWith) {
                    return;
                }
                const eqNode = parentNode;
                    ...
                    fix(fixer) {
                        ...
                        if (isStartsWith) {
                            ...
                            const posNodeIsAbsolutelyValid =
                                (posNode.type === experimental_utils_1.AST_NODE_TYPES.BinaryExpression &&
                                    posNode.operator === '-' &&
                ...
                context.report({
                    ...
                    *fix(fixer) {
                        ...
                        yield fixer.insertTextAfter(
                            argNode,
                            `${
                                callNode.type === experimental_utils_1.AST_NODE_TYPES.OptionalCallExpression
                                    ? '?.'
                                    : '.'
