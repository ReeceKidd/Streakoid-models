...
const isTruthyLiteral = type =>
    tsutils_1.isBooleanLiteralType(type, true) || (tsutils_1.isLiteralType(type) && !!type.value);
...
const isLiteral = type =>
    tsutils_1.isBooleanLiteralType(type, true) ||
    ...
    create(context, [{ allowConstantLoopConditions, checkArrayPredicates, ignoreRhs }]) {
        ...
        function checkCallExpression(node) {
            ...
            if (callback && shouldCheckCallback(node)) {
                ...
                if (
                    (callback.type === experimental_utils_1.AST_NODE_TYPES.ArrowFunctionExpression ||
                ...
                const returnTypes = tsutils_1
                    .getCallSignaturesOfType(getNodeType(callback))
        ...
        function optionChainContainsArrayIndex(node) {
            const lhsNode =
                node.type === experimental_utils_1.AST_NODE_TYPES.OptionalCallExpression ? node.callee : node.object;
            if (isArrayIndexExpression(lhsNode)) {
                return true;
            }
            if (
                lhsNode.type === experimental_utils_1.AST_NODE_TYPES.OptionalMemberExpression ||
        ...
        function checkOptionalChain(node, beforeOperator, fix) {
            ...
            const questionDotOperator = util_1.nullThrows(
                sourceCode.getTokenAfter(
                    beforeOperator,
                    token => token.type === experimental_utils_1.AST_TOKEN_TYPES.Punctuator && token.value === '?.',
                ),
