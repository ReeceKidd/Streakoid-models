...
function overrideDefine(define) {
    return function(node, definition) {
        ...
        const variable = 'name' in node && typeof node.name === 'string' && this.set.get(node.name);
        if (variable) {
...
class Referencer extends experimental_utils_1.TSESLintScope.Referencer {
    ...
    visitFunction(node) {
        ...
        if (type === experimental_utils_1.AST_NODE_TYPES.FunctionDeclaration && id) {
            upperScope.__define(
                id,
                new experimental_utils_1.TSESLintScope.Definition('FunctionName', id, node, null, null, null),
            );
        ...
        for (let i = 0; i < params.length; ++i) {
            this.visitPattern(params[i], { processRightHandNodes: true }, (pattern, info) => {
                if (pattern.type !== experimental_utils_1.AST_NODE_TYPES.Identifier || pattern.name !== 'this') {
                    innerScope.__define(
                        pattern,
                        new experimental_utils_1.TSESLintScope.ParameterDefinition(pattern, node, i, info.rest),
                    );
    ...
    TSModuleDeclaration(node) {
        ...
        if (id && id.type === experimental_utils_1.AST_NODE_TYPES.Identifier) {
            scope.__define(
                id,
                new experimental_utils_1.TSESLintScope.Definition('NamespaceName', id, node, null, null, null),
            );
        }
        this.visit(body);
    }
    ...
    TSImportEqualsDeclaration(node) {
        ...
        if (id && id.type === experimental_utils_1.AST_NODE_TYPES.Identifier) {
            this.currentScope().__define(
                id,
                new experimental_utils_1.TSESLintScope.Definition('ImportBinding', id, node, null, null, null),
            );
        }
